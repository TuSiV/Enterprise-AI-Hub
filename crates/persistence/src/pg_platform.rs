...
impl RequestRepository for PgRequestRepository {
    ...
    async fn get(&self, id: &str) -> Result<AiRequest> {
        let row = sqlx::query("SELECT * FROM ai_requests WHERE id = $1")
            .bind(id).fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::Request, e))?
    ...
    async fn list(&self, filter: &RequestFilter) -> Result<(Vec<AiRequest>, u64)> {
        ...
        if let Some(app) = &filter.application_id {
            ...
            where_clause.push_str(&format!("{}application_id = ${}", prefix_and(&where_clause), binds.len()));
        }
        if let Some(status) = &filter.status {
            binds.push(status.clone());
            where_clause.push_str(&format!("{}status = ${}", prefix_and(&where_clause), binds.len()));
        }
        if let Some(model) = &filter.model {
            binds.push(model.clone());
            let n = binds.len();
            where_clause.push_str(&format!("{}(resolved_model_key = ${n} OR requested_model = ${n})", prefix_and(&where_clause)));
        }
        if let Some(from) = filter.from {
            binds.push(from.to_rfc3339());
            where_clause.push_str(&format!("{}started_at >= ${}::timestamptz", prefix_and(&where_clause), binds.len()));
        }
        ...
        let total = count_query.fetch_one(&self.pool).await
            .map_err(|e| db_error(DomainResource::Request, e))?.get::<i64, _>("c");
        ...
        let rows = list_query.fetch_all(&self.pool).await.map_err(|e| db_error(DomainResource::Request, e))?;
    ...
    async fn usage_for_request(&self, request_id: &str) -> Result<Option<UsageRecord>> {
        Ok(sqlx::query("SELECT * FROM usage_records WHERE request_id = $1")
            .bind(request_id).fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::Request, e))?
            .map(|r| UsageRecord {
                id: r.get("id"),
    ...
    async fn cost_for_request(&self, request_id: &str) -> Result<Option<CostRecord>> {
        Ok(sqlx::query("SELECT * FROM cost_records WHERE request_id = $1")
            .bind(request_id).fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::Request, e))?
            .map(|r| CostRecord {
                id: r.get("id"),
                request_id: r.get("request_id"),
...
impl UsageRepository for PgUsageRepository {
    async fn summary(&self, query: &UsageQuery) -> Result<UsageAggregates> {
        ...
        let mut agg = agg_from_row(&q.fetch_one(&self.pool).await.map_err(|e| db_error(DomainResource::Request, e))?);
    ...
    async fn timeseries(&self, query: &UsageQuery, bucket: &str) -> Result<Vec<(String, UsageAggregates)>> {
        ...
        let rows = q.fetch_all(&self.pool).await.map_err(|e| db_error(DomainResource::Request, e))?;
        Ok(rows.iter().map(|row| (row.get::<String, _>("bucket"), agg_from_row(row))).collect())
    }
    ...
    async fn by_application(&self, query: &UsageQuery) -> Result<Vec<(String, UsageAggregates)>> {
        let mut sql = format!("SELECT COALESCE(r.application_id, 'unattributed') AS bucket, {AGG_SELECT}{USAGE_JOIN}");
        let mut binds: Vec<String> = Vec::new();
        if let Some(app) = &query.application_id {
            sql.push_str(&format!(" WHERE r.application_id = ${}", binds.len() + 1));
            binds.push(app.clone());
...
impl aihub_domain::prompt::PromptRepository for PgPromptRepository {
    async fn create(&self, p: aihub_domain::prompt::NewPrompt) -> Result<aihub_domain::prompt::Prompt> {
        ...
        sqlx::query("SELECT * FROM prompts WHERE id = $1").bind(&id).fetch_one(&self.pool)
            .await.map(|r| prompt_from_row(&r)).map_err(|e| db_error(DomainResource::Prompt, e))
    }

    async fn get(&self, id: &str) -> Result<aihub_domain::prompt::Prompt> {
        sqlx::query("SELECT * FROM prompts WHERE id = $1").bind(id)
            .fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::Prompt, e))?
    ...
    async fn get_by_key(&self, key: &str) -> Result<aihub_domain::prompt::Prompt> {
        sqlx::query("SELECT * FROM prompts WHERE key = $1").bind(key)
            .fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::Prompt, e))?
            .map(|r| prompt_from_row(&r))
    ...
    async fn list(&self) -> Result<Vec<aihub_domain::prompt::Prompt>> {
        let rows = sqlx::query("SELECT * FROM prompts ORDER BY created_at")
            .fetch_all(&self.pool).await.map_err(|e| db_error(DomainResource::Prompt, e))?;
        Ok(rows.iter().map(prompt_from_row).collect())
    }

    async fn update(&self, id: &str, name: String, description: Option<String>) -> Result<aihub_domain::prompt::Prompt> {
        sqlx::query("UPDATE prompts SET name=$1, description=$2, updated_at=$3 WHERE id=$4")
            .bind(&name).bind(&description).bind(Utc::now()).bind(id)
            .execute(&self.pool).await.map_err(|e| db_error(DomainResource::Prompt, e))?;
        self.get(id).await
    }
    ...
    async fn delete(&self, id: &str) -> Result<()> {
        sqlx::query("DELETE FROM prompts WHERE id = $1")
            .bind(id).execute(&self.pool).await.map_err(|e| db_error(DomainResource::Prompt, e))?;
        Ok(())
    }

    async fn create_version(&self, v: NewPromptVersion) -> Result<aihub_domain::prompt::PromptVersion> {
        let row = sqlx::query("SELECT COALESCE(MAX(version), 0) AS v FROM prompt_versions WHERE prompt_id = $1")
            .bind(&v.prompt_id).fetch_one(&self.pool).await.map_err(|e| db_error(DomainResource::Prompt, e))?;
    ...
    async fn versions_for(&self, prompt_id: &str) -> Result<Vec<aihub_domain::prompt::PromptVersion>> {
        let rows = sqlx::query("SELECT * FROM prompt_versions WHERE prompt_id = $1 ORDER BY version DESC")
            .bind(prompt_id).fetch_all(&self.pool).await.map_err(|e| db_error(DomainResource::Prompt, e))?;
        Ok(rows.iter().map(pv_from_row).collect())
    }

    async fn get_version(&self, version_id: &str) -> Result<aihub_domain::prompt::PromptVersion> {
        sqlx::query("SELECT * FROM prompt_versions WHERE id = $1").bind(version_id)
            .fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::Prompt, e))?
            .map(|r| pv_from_row(&r))
            .ok_or_else(|| DomainError::not_found(DomainResource::Prompt, version_id))
    }

    async fn published_version(&self, prompt_id: &str) -> Result<Option<aihub_domain::prompt::PromptVersion>> {
    ...
    async fn set_version_status(&self, version_id: &str, status: aihub_domain::prompt::PromptVersionStatus) -> Result<aihub_domain::prompt::PromptVersion> {
        sqlx::query("UPDATE prompt_versions SET status = $1 WHERE id = $2")
            .bind(status.as_str()).bind(version_id)
            .execute(&self.pool).await.map_err(|e| db_error(DomainResource::Prompt, e))?;
...
impl UserRepository for PgUserRepository {
    ...
    async fn get(&self, id: &str) -> Result<User> {
        sqlx::query("SELECT * FROM users WHERE id = $1").bind(id)
            .fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::User, e))?
            .map(|r| user_from_row(&r))
            .ok_or_else(|| DomainError::not_found(DomainResource::User, id))
    }

    async fn get_by_username(&self, username: &str) -> Result<Option<User>> {
        Ok(sqlx::query("SELECT * FROM users WHERE username = $1").bind(username)
            .fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::User, e))?
            .map(|r| user_from_row(&r)))
    }

    async fn get_by_subject(&self, provider: &str, subject: &str) -> Result<Option<User>> {
        Ok(sqlx::query("SELECT * FROM users WHERE identity_provider = $1 AND external_subject = $2")
    ...
    async fn list(&self) -> Result<Vec<User>> {
        let rows = sqlx::query("SELECT * FROM users ORDER BY created_at")
            .fetch_all(&self.pool).await.map_err(|e| db_error(DomainResource::User, e))?;
    ...
    async fn assign_role(&self, user_id: &str, role_key: &str) -> Result<()> {
        let role = sqlx::query("SELECT id FROM roles WHERE key = $1").bind(role_key)
            .fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::User, e))?
    ...
    async fn roles_of(&self, user_id: &str) -> Result<Vec<Role>> {
        let rows = sqlx::query("SELECT r.* FROM roles r JOIN user_roles ur ON ur.role_id = r.id WHERE ur.user_id = $1")
            .bind(user_id).fetch_all(&self.pool).await.map_err(|e| db_error(DomainResource::User, e))?;
        Ok(rows.iter().map(role_from_row).collect())
    }

    async fn ensure_role(&self, key: &str, name: &str, description: &str, permissions: &[&str]) -> Result<Role> {
        let existing = sqlx::query("SELECT id FROM roles WHERE key = $1").bind(key)
            .fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::User, e))?;
        ...
        for permission in permissions {
            let pid = match sqlx::query("SELECT id FROM permissions WHERE key = $1").bind(permission)
                .fetch_optional(&self.pool).await.map_err(|e| db_error(DomainResource::User, e))?
            {
                Some(row) => row.get::<String, _>("id"),
                None => {
        ...
        sqlx::query("SELECT * FROM roles WHERE id = $1").bind(&role_id)
            .fetch_one(&self.pool).await.map(|r| role_from_row(&r))
